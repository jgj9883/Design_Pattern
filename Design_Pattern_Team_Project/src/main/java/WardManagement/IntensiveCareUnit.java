 ...
 */
class IntensiveCareUnit extends Ward {
    ...
    protected ArrayList update(String id) {
        try {
            SQL = "SELECT patientid, name, dateofbirth,symptom,diagnosis,treatment,prescription FROM patient WHERE patientid Like '"+id+"%'";
            ...
            while (rs.next()) {
                ...
                map.put("treatment", rs.getString(6));
                map.put("prescription", rs.getString(7));
                list.add(map);
            }
        } catch (Exception e) {
    ...
    protected void care() {
        System.out.println("중환자 관리");
        new ICUCheckList().setVisible(true);
    }
